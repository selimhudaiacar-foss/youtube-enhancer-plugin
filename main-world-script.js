(function () {
    ...
    const CONFIG_SYNC_EVENT = "yt-enhancer-plugin:config-sync";
    ...
    const applyRuntimeConfigSync = function (nextConfig) {
        if (!nextConfig || typeof nextConfig !== "object") return;

        if ("functionState" in nextConfig && nextConfig.functionState && typeof nextConfig.functionState === "object") {
            CONFIG.functionState = nextConfig.functionState;
        }
        if ("theme" in nextConfig) {
            CONFIG.theme = nextConfig.theme;
        }
        if ("videoPlaySpeed" in nextConfig) {
            const nextSpeed = Number(nextConfig.videoPlaySpeed);
            CONFIG.videoPlaySpeed = Number.isFinite(nextSpeed) ? nextSpeed : 1;
        }
        if ("videoLoop" in nextConfig) {
            CONFIG.videoLoop = !!nextConfig.videoLoop;
        }
        if ("videoResumeMap" in nextConfig && nextConfig.videoResumeMap && typeof nextConfig.videoResumeMap === "object") {
            CONFIG.videoResumeMap = nextConfig.videoResumeMap;
        }
        if ("language" in nextConfig) {
    ...
    window.addEventListener("message", (event) => {
        ...
        if (!data || data.source !== "YTEnhancerPluginContent") return;
        if (data.type !== "SYNC_CONFIG") return;
        if (data.communicationKey !== CONFIG.communicationKey) return;

        applyRuntimeConfigSync(data.config);
        document.dispatchEvent(new CustomEvent(CONFIG_SYNC_EVENT));
    }, false);
    ...
    const AdBlock = {
        ...
        configSyncListenerAttached: false,
        ...
        scrubKnownGlobals: function () {
            ...
            try {
                this.sanitizeYtPlayerConfig(window.ytplayer);
            } catch (e) {
                console.debug("YTEnhancerPlugin AdBlock: ytplayer scrub failed", e);
            }
        },
        ...
        sanitizeYtPlayerConfig: function (ytplayerObj) {
            if (!ytplayerObj || typeof ytplayerObj !== "object") return false;
            let changed = false;

            try {
                const config = ytplayerObj.config;
                ...
                if (args && typeof args === "object") {
                    ...
                    for (const responseKey of responseKeys) {
                        const currentValue = args[responseKey];
                        if (typeof currentValue === "string") {
                            const sanitized = this.sanitizeJsonText(currentValue);
                            if (sanitized && sanitized.changed) {
                                args[responseKey] = sanitized.text;
                                changed = true;
                            }
                        } else if (currentValue && typeof currentValue === "object") {
                            if (this.pruneAdsFromData(currentValue, 0)) {
                                changed = true;
                            }
                        }
                    }
                ...
                if (config.player_response && typeof config.player_response === "object") {
                    if (this.pruneAdsFromData(config.player_response, 0)) {
                        changed = true;
                    }
                }
            } catch (_e) { }
        ...
        patchYtPlayerConfig: function () {
            if (this.ytPlayerPatched) return;
            const self = this;

            let cachedValue = window.ytplayer;
            self.sanitizeYtPlayerConfig(cachedValue);

            let descriptor;
            try {
                descriptor = Object.getOwnPropertyDescriptor(window, "ytplayer");
            } catch (_e) { }

            if (descriptor && descriptor.configurable === false) {
                this.ytPlayerPatched = true;
                return;
            }
        ...
        shouldSanitizeJsonUrl: function (url) {
            ...
            return /youtube(?:i)?\/v1\/(?:player|get_watch|next|browse|reel\/[a-z_]+)/.test(url)
        ...
        patchXMLHttpRequest: function () {
            ...
            window.XMLHttpRequest.prototype.open = function (method, url, ...rest) {
                this.__ytEnhancerPluginUrl = typeof url === "string" ? url : "";
            ...
            window.XMLHttpRequest.prototype.send = function (...args) {
                this.addEventListener("readystatechange", function () {
                    try {
                        ...
                        const url = this.__ytEnhancerPluginUrl || "";
        ...
        observeInterruptionMutations: function () {
            if (this.interruptionObserver) return;
        ...
        run: function () {
            ...
            this.patchYtPlayerConfig();
            this.patchPromiseThen();
            this.patchJsonParse();
            this.patchFetch();
            this.patchXMLHttpRequest();
            this.patchResponseJson();
            this.observeInterruptionMutations();

            if (!this.configSyncListenerAttached) {
                document.addEventListener(CONFIG_SYNC_EVENT, () => {
                    this.tick();
                }, false);
                this.configSyncListenerAttached = true;
            }

            this.tick();
            setTimeout(() => this.tick(), 60);
            setTimeout(() => this.tick(), 180);
            setTimeout(() => this.tick(), 420);
